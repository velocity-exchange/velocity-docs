...
In both formulas `size` is your own token balance in that market, not a dollar value and not the market's total. The discount is a ceiling, so it caps your initial and maintenance asset weights alike. For how the two weights differ, when each one applies, and a worked example, see [Cross-collateral deposits](/protocol/getting-started/cross-collateral-deposits).
