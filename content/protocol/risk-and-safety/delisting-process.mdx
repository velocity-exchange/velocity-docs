...
*   Funding rate updates **continue**: funding still accrues throughout reduce-only mode (a market's funding updates while its status is `Active` **or** `ReduceOnly`), so positions keep paying/receiving funding right up to expiry

*   Users cannot settle unrealized P&L prior to expiry

<Callout type="warning">
Reduce-only applies at **fill** time, not only at placement. An order you placed while the market was still `Active` carries `reduce_only = false`, but the fill path re-derives reduce-only from the live market status for the taker and for every maker in the match, so that order can no longer add exposure. See [Block Conditions](/protocol/risk-and-safety/block-conditions#layer-3-market-status).
</Callout>
...
### PnL Pool Settlement
Once the market is in Settlement status, all base exposure is closed out (no users left holding base), the net quote balance is zero, and no bankruptcy claims remain unresolved, the remaining balance in the PnL Pool can be settled into the quote asset's [Revenue Pool](/protocol/how-it-works/revenue-pool)
</Steps>
...
### Reduce only mode
The market immediately enters "reduce only mode" once an expiry date is set
*   This blocks new borrows and any deposit that does not pay down an existing borrow (there is no spot order book, so there are no spot "buys" to block)
