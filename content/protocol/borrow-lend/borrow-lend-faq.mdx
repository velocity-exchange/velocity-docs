...
*   Each spot market takes two independent cuts off of deposit-interest gains before lenders are credited: an `ifFeeFactor` carveout that goes to that market's Insurance Fund (staker-owned, see [Insurance Fund](/protocol/insurance-fund)), and a separate `protocolFeeFactor` carveout that goes to a protocol-owned, withdrawable fee pool. Lenders receive the deposit interest net of both cuts.
...
*   LTV limits are set per market by admin-controlled weights and can change over time. Check the live table in [Cross-Collateral Deposits](/protocol/getting-started/cross-collateral-deposits) for the current initial and maintenance limits.
