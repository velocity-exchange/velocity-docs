...
- AMM: backed by VLP (Velocity's LP token) rather than a split between LP-owned and protocol-owned liquidity. Increased assurance of backstop indiscriminate liquidity per market, ultimately covered by the revenue pool and the insurance fund (which is 100% staker-owned) in cases of failure.

The answers below take the auction one question at a time. Here is the whole path in one picture:
...
JIT Proxy Address: `J1TPRoXCtGuMcWiWFE6RB9eZU8U35PBMETCwNQLCNPhQ`. This is Velocity's jit-proxy program (`declare_id!` in `programs/jit-proxy/src/lib.rs`), which the client in the velocity-v1 monorepo targets. Verify the address against official Velocity channels before sending funds or transactions.
...
Yes, users can cancel the unfilled portion of their order in the auction once they've been initiated. A user cancelling their own order pays only the Solana network fee in SOL. The \$0.01 nominal keeper fee is charged only when a keeper cancels the order on the user's behalf (for example, order expiry or a reduce-only cancellation).
...
Exclusive of slippage and fees, the UI will display the price that the Taker will be filled at if they were to be filled against the AMM. The Taker's actual worst price is bounded by their own auction/limit price, not by this AMM quote: a resting Maker priced worse than the AMM can still fill part of the order, after the AMM's liquidity at that price level is consumed. See [Matching Engine](/protocol/how-it-works/matching-engine) for how fills interleave between the AMM and resting orders.
...
No, liquidations do not use auctions. Liquidators either take over user positions directly, or fill liquidated users against resting maker liquidity or the AMM's own curve.
