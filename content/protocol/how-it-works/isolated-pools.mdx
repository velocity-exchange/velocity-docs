# Isolated Pools

Velocity's isolated pools allow users to swap or borrow/lend with specific risks confined to a particular pool. Each isolated pool is segregated, meaning that the risks associated with one pool do not impact other pools on Velocity.

<Callout type="important">
An isolated-pool subaccount cannot place perpetual orders. Perp trading requires the main account (pool ID 0); isolated pools are for swaps and leveraged borrow/lend only.
</Callout>

<Callout type="info">
Not to be confused with an [Isolated Insurance Fund](/protocol/insurance-fund) (a per-market insurance backstop){/* HIDDEN: or [isolated positions](/protocol/trading/margin) (per-position perp margin) */}. Isolated pools are segregated collateral pools.
</Callout>

### What's the difference between Isolated and Cross-collateral pools?

Cross collateral (default accounts for Velocity) shares a single collateral pool across multiple positions, allowing all assets in the account to back each other. This method spreads risk and increases margin efficiency but exposes the entire account to liquidation if risks are not managed. Isolated pools, however, keep collateral and risk confined to a single pool or position. Losses or liquidation in one pool do not impact the rest of the platform or the user's other positions.

| **Feature** | **Cross Collateral** | **Isolated Pools** |
| --- | --- | --- |
| **Risk Sharing** | Shared across all positions | Confined to a single pool |
| **Account Health** | Determined by total account collateral | Customized per pool |
| **Liquidation Impact** | All positions at risk if account health fails | Only the affected pool is liquidated |
| **Margin Efficiency** | High (collateral supports all positions) | Low (collateral is pool-specific) |
| **Use Case** | Suitable for traders/stakers managing multiple positions and want to optimise capital efficiency | Ideal for users who want to swap or borrow/lend against a single deposit and contain risk to a limited set of tokens (no perp trading) |

<Callout type="info">
### Example - Cross Collateral Pool
1. **Deposit Collateral**: Deposit 1000 SOL into your account.
2. **Open a BTC Long Position**: Use some of the SOL as collateral to open a leveraged BTC long position.
3. **Open a USDT Borrow Position**: Simultaneously borrow USDT using the same SOL collateral.
4. **Risk Management**:
    - Both positions rely on the same SOL collateral.
    - If SOL's value drops significantly, both the BTC and USDT positions may face liquidation.
</Callout>

<Callout type="info">
### Example - Isolated Pool
1. **Choose an Isolated Pool**: Select an isolated pool for the asset pair you want to swap or borrow/lend against.
2. **Deposit Collateral**: Deposit collateral into the pool.
3. **Open a Leveraged Position**: Borrow against your deposit and swap into the collateral asset to increase exposure. You can earn higher yield on your initial deposit through this leveraged position.
4. **Risk Management**:
    - If the collateral asset's price drops, only this pool's position is at risk of liquidation.
    - Other pools or positions you hold remain unaffected. At the same time, you'll not be able to use the token deposited in the isolated pool for other positions.
    - Isolated pools can potentially offer higher LTV as the risk exposure is contained
</Callout>

### Supported Pools

| **Pool** | **Assets** |
| --- | --- |
| JLP Isolated Pool | JLP, USDT |

### How to deposit into an isolated pool

<Steps>
#### Navigate to isolated pool markets
Find the isolated pool markets at the bottom of the [borrow/lend page](https://velocity.exchange/earn/borrow-lend)

![Isolated Pool](/assets/isolated-pool.png)


#### Select Deposit into the isolated pool

![Select Isolated Pool](/assets/isolated-pool-select.png)

#### Enter the amount and select deposit and create account

![Deposit Into Isolated Pool](/assets/deposit-isolated-pool.png)
</Steps>

You have now created an Isolated pool account, and can borrow and lend within the isolated pool

<Callout type="warning">
*Reminder: Please take note of account health and risk of liquidation while exposed to isolated pools*
</Callout>