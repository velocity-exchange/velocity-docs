...
For cancelling orders (that can be cancelled, see [Advanced Orders FAQ](/protocol/trading/order-types/advanced-orders-faq)), Keepers will receive the `flat_filler_fee` from the market's fee structure, paid out via `pay_keeper_flat_reward_for_perps` when a keeper triggers an order expiry or reduce-only cancellation.
...
where:
$t_{\text{order}}$ is slots elapsed since the order was placed
