...
Velocity's AMM design builds on Perpetual Protocol's innovation, the [virtual AMM (vAMM)](https://blog.perp.fi/a-deep-dive-into-our-virtual-amm-vamm-40345c522eeb), with configurable parameters, namely:
...
<Callout type="info">
  JIT only fires against resting maker orders. Filling directly against the AMM's own curve still
  requires the order to be eligible for an AMM fill (see above).
</Callout>
...
The AMM can also be paid a maker rebate on the fills where it is the maker. How the rebate is computed and where it lands:
...
The taker pays exactly the same fee whether the flag is on or off. What changes is who receives that slice of the fee: with the flag on, part of the protocol and insurance-fund legs is redirected to the AMM. See [Trading Fees](/protocol/trading/trading-fees/fee-mechanics#vamm-maker-rebate) for the full fee waterfall.
