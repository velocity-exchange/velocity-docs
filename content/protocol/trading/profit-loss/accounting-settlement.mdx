...
Every drain reserves three live claims in the P&L Pool: `max(net_user_pnl, 0)` (users' positive unsettled P&L, valued at the fixed expiry price rather than the live oracle while the market is in Settlement), the floored insurance-fund bankruptcy tranche (frozen entirely while a bankruptcy claim is latched), and any `pending_revenue_share` already accrued and owed to builders or referrers. The protocol's cut sweeps first and is exempt from the per-market retention buffer (`fee_pool_buffer_target`); the insurance-fund cut and AMM-provision tokenization sweep next, above that buffer.
</Callout>
