...
<Callout type="warning">
  Trading on leverage can be risky. Make sure you are aware of the liquidation rules outlined below
  before applying margin.
</Callout>
...
The user's trade price at liquidation for a perpetual position will be based on the oracle price. Both the liquidator fee and the insurance-fund fee are per-market parameters set by protocol admins, and the liquidator fee is what gives the liquidator a rebate for taking over the position.

<Callout type="error">
  **In the case of an extreme oracle error: the market will pause most fills, liquidations, and
...
- Margin requirement = `5000 * .03 = 150`

You are liquidated if the price of SOL decreases by ~7.22% to ~\$23.20. This is because:

- Perp PnL = `200 * $25 * -0.0722 = -361`
- New total collateral = `500 - 361 = 139`
- New margin requirement = `23.20 * 200 * .03 = 139.2`
...
- **`protocol_liquidation_fee`**: a protocol cut, introduced alongside the [fee redesign](/protocol/trading/trading-fees/fee-mechanics#fee-redesign). The combined insurance-side budget (`if_liquidation_fee + protocol_liquidation_fee`) is computed once against the user's margin shortfall, then split **IF-first**: the insurance fund receives exactly what it would have received without the protocol fee, and the protocol only captures margin headroom _beyond_ that. This means adding a protocol cut can never push a liquidation into bankruptcy that wouldn't otherwise have happened. It only reduces the excess margin the liquidatee would have kept.
...
If a liquidation leaves a user with an outstanding liability and no further assets, the account becomes bankrupt and is resolved via `resolve_perp_bankruptcy` / `resolve_spot_bankruptcy` (or `resolve_perp_pnl_deficit` for a market-wide PnL deficit short of full bankruptcy). See [Insurance Fund](/protocol/insurance-fund) for the backstop waterfall itself.
