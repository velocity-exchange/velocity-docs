...
- **`protocol_liquidation_fee`**: Per-market liquidation cut routed to the withdrawable `protocol_fee_pool`, introduced with the [fee redesign](/protocol/trading/trading-fees/fee-mechanics#fee-redesign). Split IF-first against the same insurance-fee budget so it can never push a liquidation into bankruptcy. See [Liquidations](/protocol/trading/liquidations#liquidation-fees).
