...
Velocity does not have High Leverage Mode. The instructions, `User.marginMode`, and the per-market high-leverage margin ratios were removed, so there is no per-account leverage toggle beyond the per-market/per-position limits below.
</Callout>
