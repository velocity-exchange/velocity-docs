...
There is no Python SDK. The Rust client (`velocity-rs`, in a separate `rust/` workspace alongside `keep-rs` and `swift`) is source-only and not on crates.io, and `@velocity-exchange/jit-proxy` is vendored in the monorepo rather than released to npm, so it has to be built from source.
