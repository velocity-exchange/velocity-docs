...
Taker fills follow **price-priority interleaving** — there is no fixed JIT → DLOB → AMM waterfall. The fill plan is built by `determine_perp_fulfillment_methods` (`programs/velocity/src/math/fulfillment.rs`):

<Steps>
### Walk the price-sorted DLOB maker list

Resting limit orders are sorted by **price-time priority**: best price wins, and among tied prices, the earliest order fills first. DLOB liquidity is "committed": the orders are onchain and can't be pulled during matching. At each maker level, the engine compares the AMM's quote against that maker's price:
- If the AMM is better, it inserts an AMM fill ahead of the maker — **capped exactly at that maker's price level** — and the maker fills after.
- Otherwise the maker fills first.

The AMM never jumps ahead of the whole book at once; it only jumps ahead of the specific price levels it beats.

### JIT competes inside the match, by price

JIT is not a separate earlier stage. When a DLOB match step executes (`matching.rs`), JIT maker quotes — and, gates permitting, an AMM-JIT quoter — are spun up alongside the resting maker, and all compete for that same fill purely on price (ties broken by a priority flag). The taker's auction window (~first 10 slots / ~5 seconds) bounds acceptable fill prices: the auction price interpolates from the start price toward the end price, creating a Dutch auction dynamic that lets makers price off the latest oracle and inventory rather than stale resting quotes. See [JIT Auctions](./jit-auctions) for the full auction lifecycle and pricing math.

### Residual AMM catch-all

After the maker list is exhausted, any remaining AMM liquidity that still crosses the taker's limit price executes in one final AMM fill against the constant product curve (with its spread and price impact). This is the only sense in which the AMM comes "last" — by that point it's simply the only liquidity still crossing.
</Steps>

Whether the AMM may participate as a **JIT maker inside a DLOB match** is subject to hard gates: `AmmFill` isn't paused for the market, the AMM doesn't have too much drawdown, and the oracle (including the MM oracle, when active) is valid and not too volatile. If any of these gates trips, the AMM sits out of that DLOB match entirely — the fill matches against DLOB-resting orders only, with no AMM JIT competition. This is a hard, all-or-nothing gate distinct from the auction-timing rules that govern whether the AMM fills a *fresh* taker order immediately (see [JIT Auctions](./jit-auctions#amm-as-a-jit-maker)).
...
`/l2` serves the server's Redis-cached book; whether vAMM liquidity is included in that cache is decided server-side, not by a query parameter. `includeVamm` (along with `includePhoenix`/`includeOpenbook`/`includeOracle`) is a parameter of the separate `/batchL2`/`/batchL2Cache` endpoints, not `/l2`.
