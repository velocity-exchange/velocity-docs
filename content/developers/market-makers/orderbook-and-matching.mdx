...
[PropAMM Order Flow](/developers/concepts/propamm-order-flow).
</Callout>
...
For the full model — the quoter interface, the registry, and the account limits a client must respect — see [PropAMM Order Flow](/developers/concepts/propamm-order-flow). That page calls a liquidity source a **quoter**; this page keeps "source" because the L2 API field is named `sources`.
