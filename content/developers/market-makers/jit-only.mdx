...
- `@velocity-exchange/jit-proxy`: JIT proxy SDK with `JitterSniper` and `JitterShotgun`. Unpublished: build from `packages/jit-proxy` in the `velocity-v1` monorepo, which is not public yet
