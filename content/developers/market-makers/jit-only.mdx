...
JIT-only market making means you **do not keep a standing book**. Instead of resting limit orders on the DLOB, you compete in [JIT auctions](/protocol/how-it-works/jit-faq) (see [Matching Engine](/protocol/how-it-works/matching-engine) for liquidity sources) by reacting to incoming taker orders in real-time.
...
Instead of building fill logic from scratch, use the [`@velocity-exchange/jit-proxy`](https://github.com/velocity-exchange/velocity-v1/tree/master/packages/jit-proxy) library which handles auction timing, transaction building, and retry logic. **Not yet published to npm** — build it from `packages/jit-proxy` in the monorepo until it is.
...
See the [`JitMaker`](https://github.com/velocity-exchange/velocity-v1/blob/master/apps/keeper-bots-v2/src/bots/jitMaker.ts) bot for a complete production example using `JitterSniper`/`JitterShotgun` with:
...
- [@velocity-exchange/jit-proxy](https://github.com/velocity-exchange/velocity-v1/tree/master/packages/jit-proxy) - JIT proxy SDK with `JitterSniper` and `JitterShotgun`
