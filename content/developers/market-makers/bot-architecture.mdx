...
Production market making bots need more than just order placement logic. This page covers the infrastructure patterns used across Velocity's [keeper-bots-v2](https://github.com/velocity-exchange/velocity-v1/tree/master/apps/keeper-bots-v2), applicable to market making, filling, and triggering bots.
...
**Tip:** Place-and-make transactions typically use 200-400k compute units. Set the limit explicitly rather than defaulting to 200k (which may fail) or 1.4M (which overpays). This is not a hard ceiling: the `JitMaker` bot runs a heavier place-and-make path and defaults to 800k CU, see [JIT Auctions gotchas](./jit-auctions#gotchas).
...
**Oracle validation**: Reject if oracle is stale, invalid, or confidence is too wide. `MMOraclePriceData` has no `isValid` field -- use `isOracleValid` (the same AMM-fill-oriented gate the program uses) against the market and the state's oracle guard rails. Pass the live slot to both calls: `getMMOracleDataForPerpMarket` needs it to classify a staged slot-duration switch correctly (omitting it falls back to a best-effort observed slot that can stall while a market is idle), and `isOracleValid` needs it plus the state's configured slot duration for its staleness window, so the threshold matches the on-chain check once a shorter slot duration is staged:
...
The [`JitMaker`](https://github.com/velocity-exchange/velocity-v1/blob/master/apps/keeper-bots-v2/src/bots/jitMaker.ts) enforces a 1:1 subaccount-to-market ratio and throws if the mapping isn't configured correctly.
...
- **Priority fee spikes**: during congestion, priority fees can spike 100x. Construct `PriorityFeeSubscriber` with `maxFeeMicroLamports` (and, if needed, `priorityFeeMultiplier`) so it enforces the cap itself before returning a strategy result, instead of draining your SOL balance on fees.
...
See [keeper-bots-v2](https://github.com/velocity-exchange/velocity-v1/tree/master/apps/keeper-bots-v2) for the reference implementation of these patterns. Key files:
- [`floatingMaker.ts`](https://github.com/velocity-exchange/velocity-v1/blob/master/apps/keeper-bots-v2/src/bots/floatingMaker.ts): mutex guards, slot-based cooldowns, watchdog timer
- [`jitMaker.ts`](https://github.com/velocity-exchange/velocity-v1/blob/master/apps/keeper-bots-v2/src/bots/jitMaker.ts): subaccount isolation, DLOB integration, volatility checks
- [`utils.ts`](https://github.com/velocity-exchange/velocity-v1/blob/master/apps/keeper-bots-v2/src/utils.ts): throttling, market type conversion, volatility detection helpers
