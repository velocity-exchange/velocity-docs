...
- **ed25519 instruction ordering:** the ed25519 verification instruction must sit at the index the place instruction references, not necessarily first. `placeAndMakeSignedMsgPerpOrder`/`getPlaceAndMakeSignedMsgPerpOrderIxs` accept `precedingIxs` (instructions that will come before these in the final transaction, used only to compute the verify instruction's sysvar index) and `overrideCustomIxIndex` for an explicit override. If you're prepending anything yourself, such as compute-budget instructions, pass `precedingIxs` so the SDK computes the right index; getting this wrong causes silent verification failures.
