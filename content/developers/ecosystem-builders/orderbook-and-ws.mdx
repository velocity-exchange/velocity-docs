...
<Callout type="warning">
Prices and sizes are **raw fixed-precision integer strings**, not decimals: prices are in `PRICE_PRECISION` (1e6, so `"81114210"` = $81.11) and sizes in `BASE_PRECISION` (1e9). Each level's `sources` object splits the size by origin (`dlob`, `vamm`, `indicative`) — it is an object, not a string. See [Precision & Types](/developers/velocity-sdk/precision-and-types) for conversions.
</Callout>
