...
Extension only ever appends. It never reorders, inserts, widens, or shrinks a field, so offsets already in use stay valid, and newly added bytes read as zero until code writes them. See [Account Model](/developers/concepts/account-model#account-sizes-can-grow).
</Callout>
...
<SDKDoc>
  <TypeScript name="fetch" type="example">
```js
