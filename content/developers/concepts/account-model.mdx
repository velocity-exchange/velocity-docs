...
Velocity extensively uses PDAs for deterministic address generation. Most seed strings are unchanged from Drift, but the State seed became `"velocity_state"`, and since the **program ID is new**, every derived address is different from Drift's:
