...
- **Hedge config** (`hedgeConfig`) - this market's relationship to its hedging [VLP pool](/developers/vault-managers) (`poolId`, `status`, `pausedOperations`, `exchangeFeeExclusionScalar`, `feeTransferScalar`), replacing the earlier flat `lp*` LP-share fields
...
- **Equity floor** - `equityFloor` (admin-set minimum cross-margin collateral, 0 = disabled) and `equityFloorBuffer` (extra headroom required by risk-increasing orders, fills, withdrawals, and transfers)
...
interface UserAccount {
  ...
  equityFloor: BN;                   // QUOTE_PRECISION (1e6); admin-set minimum cross-margin collateral; 0 = disabled
  equityFloorBuffer: BN;             // QUOTE_PRECISION (1e6); extra headroom above equityFloor for risk-increasing actions
}
```
...
- **Equity breaker** - `equityBreakerTripped`, set by the permissionless equity-floor breaker across every subaccount of the authority until the warm admin clears it
...
interface UserStatsAccount {
  ...
  equityBreakerTripped: number;      // non-zero once the permissionless equity-floor breaker fires; cleared only by the warm admin
}
```
...
InsuranceFundVault PDA: ["insurance_fund_vault", marketIndex as u16 LE]
```
