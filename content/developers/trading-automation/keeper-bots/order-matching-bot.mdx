...
Market orders first go through [JIT Auctions](/protocol/how-it-works/matching-engine). Most orders become available to matching bots once the auction period ends, but the AMM can also skip the rest of the auction early when its inventory is low and it has been profitable since the last funding update, so a crossing DLOB maker or the AMM itself can fill an order before the auction ends. The DLOB exposes a `findNodesToFill` method that returns eligible orders, using the current virtual bid/ask and oracle price to determine which orders can be matched.
