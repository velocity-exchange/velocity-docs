...
Market orders first go through [JIT Auctions](/protocol/how-it-works/matching-engine). After the auction period ends, they become available for matching bots to fill. The DLOB exposes a `findNodesToFill` method that returns these eligible orders, using the current virtual bid/ask and oracle price to determine which orders can be matched.
