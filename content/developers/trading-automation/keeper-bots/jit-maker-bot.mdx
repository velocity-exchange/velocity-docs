...
This tutorial shows how to run a JIT Maker bot in Typescript from the [keeper-bots-v2 repo](https://github.com/velocity-exchange/keeper-bots-v2). A similar example written in Python is available in [keepyrs](https://github.com/velocity-exchange/keepyrs).

Market orders go through the [Just-In-Time (JIT) Auctions](/protocol/about-v3/matching-engine) where Makers fight to fill orders before the order is allowed to fill against the [Drift AMM](/protocol/about-v3/drift-amm).
...
```
git clone https://github.com/velocity-exchange/keeper-bots-v2.git
```
...
The [`jit-maker-config.yaml`](https://github.com/velocity-exchange/keeper-bots-v2/blob/master/jit-maker-config.yaml) file is good starting point. You will need to fill in the following values:
...
This bot uses the [`JitterShotgun`](https://github.com/velocity-exchange/jit-proxy/blob/master/ts/sdk/src/jitter/jitterShotgun.ts#L13) strategy and the [`jit-proxy`](https://github.com/velocity-exchange/jit-proxy) program to try to fill jit maker fills.
