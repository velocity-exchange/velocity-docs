...
Velocity Builder Codes establish the financial infrastructure layer on Solana, by allowing anyone tap into Velocity’s deep liquidity and efficient execution, without building their own backend. This enables a fully open, composable UI layer for perps on Solana.
...
In order to receive fees, builders are required to have an existing Velocity account, as well as set up a `RevenueShareAccount`
