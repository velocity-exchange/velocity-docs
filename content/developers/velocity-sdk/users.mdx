...
A **user account** is the onchain account that holds your positions, orders, and collateral on Velocity. Each wallet can create multiple user accounts (called **subaccounts**), identified by a numeric ID (0, 1, 2, etc.). Each subaccount is its own separate account with its own independent margin calculation: collateral and positions in subaccount 0 do not back or affect subaccount 1. Cross-margin applies only *within* one subaccount, where its positions and balances are netted together.
...
Subaccounts are useful for separating strategies, isolating risk between different trading styles, or delegating specific accounts to bots while keeping others manual. You can switch between subaccounts using the SDK, and each one maintains its own independent positions, orders, and collateral. Subaccounts share only the wallet authority and the `UserStats` account (volume, fee tier, delegate permissions).
