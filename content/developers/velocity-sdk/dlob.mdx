...
<Callout type="warning">
`dlob.getL2()` on its own returns resting-limit maker liquidity only, it does not include the vAMM. For a book that matches what a taker actually trades against, call `dlobSubscriber.getL2({ ..., includeVamm: true })` instead, or pass `getVammL2Generator(...)` output as `fallbackL2Generators` if you're calling `dlob.getL2()` directly.
</Callout>
