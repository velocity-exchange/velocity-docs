...
Each subaccount is its own cross-margin account: the deposits inside one subaccount back only that subaccount's positions. Collateral is not shared across subaccounts under the same wallet, which is why moving it between them requires an explicit transfer.
...
Once opted in, a delegate can call `transferDepositByDelegate`. It takes the same first four arguments as `transferDeposit`, plus an `equityFloorDelta` (`BN | 'auto'`, defaults to zero) before the trailing `txParams`, don't pass `txParams` positionally in that fifth slot:
