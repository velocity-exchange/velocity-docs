...
This page covers the taker side: signing and submitting Swift orders. For the maker side (receiving and filling signed orders over WebSocket), see the [SWIFT API guide](/developers/market-makers/swift-api).

<Callout type="info">
The SDK's own defaults are `wss://swift.velocity.exchange/ws` on mainnet and `wss://swift.master.velocity.exchange/ws` on devnet. The HTTP submit endpoint below uses the same mainnet host.
</Callout>
