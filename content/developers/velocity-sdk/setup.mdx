...
Velocity uses the same program ID on devnet and mainnet-beta. Velocity is an entirely new deployment, so user accounts must be re-initialized and balances start fresh; no prior on-chain state carries over.
...
On devnet, spot market index 0 is `dUSDT` (a placeholder quote token, 1e6 precision) rather than real USDT. Mint it from the SDK's `TokenFaucet` helper before depositing. On mainnet-beta the quote asset is USDT (`Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB`). If you're porting an integration that assumed a different quote asset, see the [migration guide](/developers/migrate-from-drift).
...
| `txHandler` | Builder/signer used for every transaction | Yes | A `TxHandler` on this connection and wallet |
