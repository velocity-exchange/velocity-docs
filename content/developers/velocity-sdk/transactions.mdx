...
- **`TxHandler`** builds and signs transactions. It resolves the blockhash, injects the compute-budget instructions, compiles address lookup tables, and calls the wallet.
- **A `TxSender`** broadcasts and confirms them. Four implementations ship with the SDK, each with a different retry and confirmation model.

`VelocityClient` creates both for you, and you can replace either through its config:

<SDKDoc>
  <TypeScript name="VelocityClient" type="class">
```js
import { VelocityClient, WhileValidTxSender, TxHandler } from "@velocity-exchange/sdk";
...
If you pass nothing, `VelocityClient` builds a `TxHandler` from its own connection and wallet, then wraps it in a `RetryTxSender`. Default `txParams` are `computeUnits: 600_000` and `computeUnitsPrice: 0`, so by default you pay no priority fee.
...
| `WhileValidTxSender` | Same resend loop as `RetryTxSender`, and additionally records each in-flight signature's blockhash and `lastValidBlockHeight` in its `untilValid` map | 2,000 ms | 35,000 ms (inherited from `BaseTxSender`; not a constructor option) | You want per-transaction blockhash validity bookkeeping available to your own code, for example a UI that reports "expired" separately from "dropped". |
...
| `ForwardOnlyTxSender` | Never calls `sendRawTransaction` on `connection`. Hands the base58 transaction to `additionalTxSenderCallbacks` and re-invokes them on an interval. `connection` is used only to confirm | 5,000 ms | 35,000 ms | Submission is delegated to a relay or bundler (for example a Jito bundle submitter) and you still want the SDK's confirmation and land-rate machinery. |
...
```js
import { FastSingleTxSender } from "@velocity-exchange/sdk";
...
const velocityClient = new VelocityClient({ connection, wallet, txSender });
```
  </TypeScript>
</SDKDoc>
...
`TxHandler` owns blockhash resolution. By default it uses a caching fetcher that reuses a recent blockhash for 2,000 ms, which collapses per-build `getLatestBlockhash` calls during bursts. Blockhashes stay valid on-chain far longer than that window, so this is safe.
...
```js
import { TransactionParamProcessor } from "@velocity-exchange/sdk";
...
const velocityClient = new VelocityClient({
  ...
  txParams: {
    ...
    getCUPriceFromComputeUnits: (computeUnits) =>
...
|---|---|---|---|
| `SOLANA` | `getRecentPrioritizationFees` on your RPC | `connection` | Samples are filtered to the most recent `slotsToCheck` slots (default 50) relative to the newest returned slot, then sorted descending by slot. Override the call itself with `fetchSolanaPriorityFee` to route through your own cache. |
...
The constructor throws if you select `HELIUS` without a resolvable Helius URL, or `SOLANA` without a `connection`.

`addresses` scopes SOLANA and HELIUS sampling to the accounts your transaction will write-lock, which is what you want: it measures congestion on the markets you are trading, not the whole cluster.
...
With `trackTxLandRate: true`, the sender records each signature and computes a rolling land rate. Feed the suggested multiplier into your compute-unit price to back off automatically under congestion:
...
- [SDK Internals](/developers/velocity-sdk/sdk-internals) - subscription modes and instruction building
- [Orderbook + DLOB websocket](/developers/ecosystem-builders/orderbook-and-ws) - the hosted `/priorityFees` and `/batchPriorityFees` endpoints
