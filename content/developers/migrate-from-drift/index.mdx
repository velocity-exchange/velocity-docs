...
| npm package | `@drift-labs/sdk` (`2.163.0-beta.0`) | `@velocity-exchange/sdk` (`0.13.x`) |
...
| Data API host | `data.api.drift.trade` | `data.velocity.exchange` (provisional) |

<Callout type="info">
This page covers the **TypeScript SDK** at `@velocity-exchange/sdk` 0.13.0. Rust (`velocity-rs`) and raw-ABI integrators should also read the [full migration reference](https://github.com/velocity-exchange/velocity-v1/blob/master/docs/DRIFT-TO-VELOCITY.md) in the monorepo.
</Callout>
...
- **Fast-fill auctions (DLOB server v3).** At API version ≥ 3, **every** market gets `isFastFill = true`: the auction starts inside the touch (`bestOffer` with a `-0.05` price offset) and lasts 2000 ms of wall clock, converted to slots at the market's active slot duration (5 slots at the 400 ms baseline, more at a shorter slot duration). Auctions become marketable almost immediately, so an MM quoting off stale prices gets run over. Derive the auction window from the live slot duration rather than hardcoding 5 slots, and use the new `generatedAt` response field for staleness checks.
...
- **MM-oracle native writes are throttled.** `update_mm_oracle_native` **silently skips** the write (returns `Ok`, no error) on a non-positive price, a sequence id that has not strictly increased, a slot that has not advanced, a resubmit inside an 800 ms write gap, or a source slot more than 800 ms away from the landing slot in either direction. A per-write price step beyond 1% is handled differently: it is not skipped, it is **clamped to the 1% cap and written**, so a crank feeding a large move converges over a few writes rather than freezing the oracle. MM crank bots that push updates faster than the 800 ms gap, or with a stale/mismatched source slot, get silent skips with no error signal. Throttle writes to at least 800 ms apart and expect an oversized step to land clamped, not rejected.
...
- **Fee cost changes for referred and staked users.** The gov-token stake discount is gone. Perp fee tier is now based on 30-day volume, floored by the admin-set `State.promo_fee_tier` (0 means no floor), so an MM that staked for a discount now pays the tiered fee for its volume, or the promo floor if higher. Separately, `VelocityClient.getMarketFees` now subtracts the referee discount from the taker fee when the client's `UserStats` shows the referred bit, and `User.calculateFeeForQuoteAmount` is renamed `User.calculatePerpTakerFee` (gaining an optional `builderInfo` arg and rounding predicted fees **up** rather than down). Re-model taker fees without any stake benefit; expect the discounted/augmented value.
...
This page reflects `@velocity-exchange/sdk` **0.13.0** (August 2026) and covers the TypeScript SDK. Rust (`velocity-rs`) and raw-ABI integrators should consult [`docs/DRIFT-TO-VELOCITY.md`](https://github.com/velocity-exchange/velocity-v1/blob/master/docs/DRIFT-TO-VELOCITY.md) in the velocity-v1 repository, which is the canonical layout-and-ABI reference.
