...
The Data API is live at `https://data.velocity.exchange`. See [Data API](/developers/data-api). Some columns below describe features Velocity removed (noted inline) and are kept here only for reference until Velocity's own Data API schema is confirmed.
</Callout>
...
| marketIndex     | int       |                     | Perpetual contract market index. See the SDK's market config (`@velocity-exchange/sdk` `configs`) for the current index-to-symbol mapping. |
...
| quoteAssetAmountSurplus                    | int                               |           | The vAMM's quote surplus on the fill, kept by the LPs: its spread capture (the curve versus the reference price), plus — when it shaded its price toward the book — the gap it held back between the price the taker paid and the live curve. Not related to unfilled size. |
...
| ~~periodRevenue~~              | int  |           | **Removed on Velocity.** The on-chain `FundingRateRecord` no longer carries this field. |
| baseAssetAmountWithAmm         | int  |           | Total base asset in the AMM pool.             |
| ~~baseAssetAmountWithUnsettledLp~~ | int  |           | **Removed on Velocity.** vAMM LP shares were removed, and the on-chain `FundingRateRecord` no longer carries this field. |
