...
**Provisional endpoint.** Velocity's hosted Data API host is not yet finalized — see [Data API](/developers/data-api). Some columns below describe features Velocity removed (noted inline) and are kept here only for reference until Velocity's own Data API schema is confirmed.
</Callout>
...
| quoteAssetAmountSurplus                    | int                               |           | The vAMM's quote surplus on the fill: the difference between the quote amount the curve actually moved and the quote at the reference price (the AMM's spread capture). Not related to unfilled size. |
